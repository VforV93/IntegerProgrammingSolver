...
class Tableau:
    ...
    def step(self, rule=None):
        ...
        i = pivot[0]
        p = self.A[i, j]
        print(f"Individuato Pivot:{p} in colonna: {j}, riga: {i+1}")
    ...
    def __str__(self):
        s = []
        form = (len(self.c))*' {:<5} |'
        s.append('\x1b[0;30;41m' + form.format(*self.c) + '\x1b[0m' + '\x1b[0;30;42m {:<5} \x1b[0m'.format(self._z))
        for i, row in enumerate(self.A):
            riga = ""
            for j in range(0, self.A.shape[1]):
                r = ' {:<5} |' if j not in self.B else '\x1b[0;30;44m {:<5} |\x1b[0m'
                riga = riga + r
            s.append(riga.format(*row) + '\x1b[0;30;43m {:<5} \x1b[0m'.format(self.b[i]))
